 ...
 * lets a timer expire. Two silences mean different things and are timed differently. Before
 * anything has arrived the pack may still be preparing the dump, so the first answer gets a long
 * grace; once bytes are flowing, a much shorter gap means the burst is over. A ceiling bounds both,
 * so a pack that trickles forever still yields a measurement rather than hanging the UI.
 */
...
/**
 * How long total silence is allowed to run before the read is called unanswered. Generous, because
 * calling a slow pack silent is the one mistake this diagnostic must not make: the whole question
 * is whether the opcode produces anything at all.
 */
const FIRST_ANSWER_MS = 8_000

/**
 * A gap this long once bytes are flowing ends the read. Back-to-back 300-byte frames arrive orders
 * of magnitude faster than this at any MTU, so two seconds of nothing is the burst finishing.
 */
...
export class DetailLogRun {
  ...
  /** Every frame that assembled in the window, whatever its type. */
  ...
  readonly stop = (): void => {
    ...
    settle({
      outcome: detailLogOutcome(this.notificationBytes, this.headers),
