/**
 * The leading fields of a frame, read for every frame that lands inside a stored-log read rather
 * than only for the ones that decode.
 *
 * On a type 0x06 frame these are the header the detail-log layout specifies, and a whole burst's
 * worth of them off one run is what shows how the pack chunked its answer. On any other frame type
 * they are simply the bytes that sit at those offsets, kept so that a reply which is not a detail
 * log still shows what it was.
 */
export interface DetailLogFrameHeader {
  /** Byte [4]: which frame this is. 0x06 is the stored detail log. */
  readonly frameType: number
  /**
   * Byte [5]: unidentified. It changes from frame to frame of a burst with no sequence to it, so
   * nothing may order or de-duplicate frames by it. Carried because it is the one header byte whose
   * meaning is still open, and it cannot be studied if it is not surfaced.
   */
  readonly unidentifiedByte: number
  /** Bytes [6..7], uint16 LE: ring index of the first record the frame carries. */
  readonly firstRecordIndex: number
  /** Byte [8]: how many records the frame claims to carry. */
  readonly recordCount: number
}
