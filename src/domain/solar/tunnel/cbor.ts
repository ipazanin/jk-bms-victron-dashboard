 ...
 *
 * CBOR is not a Victron format, so nothing here may be reused for VE.Direct over serial or
 * for the Instant Readout advertisement. Both of those are little-endian all the way down.
 * A CBOR head is big-endian because RFC 8949 says so, and the two must never share a helper:
 * the moment one function serves both links, one of them is wrong and neither test can say
 * which.
 *
 * Register ids, status codes and byte-string lengths are all read by the same routine,
 * `readCborArgument`, because in CBOR they are the same construct — the low five bits of a
 * head byte say how many argument bytes follow, whatever the major type is. That is
 * deliberate. Reading 0x58 as a value two bytes wide, which is what one public
 * implementation does, is only possible if lengths are looked up in a table of head bytes
 * instead of being decoded; sharing the decoder makes the mistake unavailable, and it also
 * gets 0x50 right for free, which a mask of the low four bits does not.
 *
 * Readers return null when the buffer ends mid-item and throw when the bytes are not a form
 * this module understands. The caller needs both: the first means wait for the next
 * notification, the second means resynchronise.
 */
...
/**
 * A ceiling on a byte string, so a corrupt length cannot make the reassembly buffer grow
 * without bound. It is not a CBOR limit and not a documented Victron one: the largest value
 * captured anywhere is a 52-byte history record, and this leaves an order of magnitude of
 * headroom above it.
 */
...
/**
 * The argument of the CBOR item whose head byte sits at `offset`, and where the item's
 * content begins.
 *
 * Big-endian, and not because Victron chose it — RFC 8949 fixes network byte order for
 * every argument in the encoding. Written out by hand rather than through a DataView so the
 * byte order is visible at the point it matters instead of hiding in a boolean argument.
 */
function readCborArgument(bytes: Uint8Array, offset: number): TunnelReading<number> | null {
  ...
  // Four- and eight-byte arguments, the reserved values, and the indefinite-length form all
  // land here. Each is well-formed CBOR that the tunnel has never been observed using, and
  // guessing a width for one of them is how a reader walks off the end of a PDU.
 ...
 *
 * Big-endian, which is inverted from VE.Direct over serial, where the same register id goes
 * out low byte first. The bytes inside the value the register returns stay little-endian.
 * Nothing in the encoding hints at that split, so it lives here, in the one function that
 * produces register bytes for this link.
 */
...
/**
 * Reads a register id or a status code — the tunnel spells both as a CBOR unsigned integer,
 * and the position in the PDU is what tells them apart.
 *
 * Accepts the immediate form a canonical encoder would use below 24, which
 * `encodeCborRegisterId` never emits for a register: the tunnel spells even small registers
 * out as `18 <reg8>`, while a status code of 1 does arrive as the single byte 0x01.
 */
